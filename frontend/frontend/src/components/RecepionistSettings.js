...
const AdminSettings = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const receptionistResponse = await fetch(`http://localhost:8000/api/recepionists/?email=${data.email}`, {
        headers: {
          'Authorization': `Token ${token}`,
          'Content-Type': 'application/json',
        },
      ...
      if (receptionistResponse.ok) {
        const receptionists = await receptionistResponse.json();
        if (receptionists.length > 0) {
          const receptionist = receptionists[0];
          setProfile({
            name: receptionist.name || data.name || '',
            email: receptionist.email || data.email || '',
            phone: receptionist.phone || '',
            recepionistid: receptionist.recepionistid || '',
          });
        } else {
          setProfile({
            name: data.name || '',
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/receptionist/profile/', {
        method: 'PATCH',
        headers: {
          'Authorization': `Token ${token}`,
          'Content-Type': 'application/json',
  ...
  if (loading) {
    return (
      <RecepionistSideNavbar activeMenu="settings">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <FaUserCheck className="text-blue-600" /> Admin Settings
          </h1>
            ...
            <div className="text-center">
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="flex items-center justify-between text-sm py-2">
                  ...
                  <span className="font-medium text-gray-700">#{profile.recepionistid}</span>
                </div>
                <div className="flex items-center justify-between text-sm py-2">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <p className="font-medium text-gray-800">#{profile.recepionistid}</p>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg">
